...
public interface StockPricesRepository extends JpaRepository<StockPrices, Integer> {
	...
	List<StockPrices> findByCompanyCodeInOrderByDate(List<String> codes);

	List<StockPrices> findByCompanyCodeOrderByDate(String code);

	@Query(value = "SELECT * FROM stock_prices as a WHERE a.company_code IN ?1 AND a.date_ BETWEEN ?2 AND ?3", nativeQuery = true)
	List<StockPrices> findStockInRange(List<String> codes, Date date, Date date2);
	
	List<StockPrices> findByStockCode(String stockCode);
}